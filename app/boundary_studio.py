...
def _render_prediction_lab(case_manifest: pd.DataFrame) -> None:
    ...
    if submitted:
        command = [
            ...
            "--config",
            "configs/double_pipe_3d_synthetic_conditioned_validation_optphys.yaml",
            "--checkpoint",
            "outputs_3d_synthetic_qagg_positivep_optphys2_10ep_dpcal_walltune/checkpoints/best_model_3d.pt",
            "--temperature-calibration-json",
            "outputs_3d_synthetic_qagg_positivep_optphys2_10ep_dpcal_walltune/boundary_temperature_calibration.json",
            "--Th-in",
            str(Th),
            "--Tc-in",
            str(Tc),
    ...
    if submitted:
        if preset == "Quick check":
            ...
            command = [
                ...
                "--config",
                "configs/double_pipe_3d_synthetic_case001.yaml",
                "--adam-epochs",
                "8",
                "--set",
                f"synthetic_3d.case_id={case_id}",
                "--set",
